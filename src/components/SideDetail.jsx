...
import { setRoute } from "../redux/Slices/flightSlice";
  ...
  useEffect(() => {
    // ucus id`si her degistiginde onceki detay verilerini sil
    // bu sayede gecislerde loading tetiklenir.
    setDetail(null);
      ...
      <div className="detail-inner">
        ...
        {!d ? (
          ...
          <>
            ...
            <p>
              <span>Departure:</span>
              <a target="_blank" rel='noreferrer' href={d.airport.origin?.website}>
                {d.airport.origin?.name}
              </a>
            </p>
            <p>
              <span>Destination:</span>
              <a target="_blank" rel='noreferrer' href={d.airport.destination?.website}>
                {d.airport.destination?.name}
              </a>
            </p>
            <p>
              <span>Status: </span>
              <span className="status" style={{background: d.status?.icon}}>{d.status?.text}</span>
            </p>
          </>
        )}
      </div>
